...
def game_loop(args):
    ...
    try:
        ...
        while True:
           ...
           # if (world.player.get_transform().location.x>-85 and world.player.get_transform().location.x<-82 and world.player.get_transform().location.y>90 and world.player.get_transform().location.y<120):

             
              
            
            if (world.player.get_transform().location.x>-85 and world.player.get_transform().location.x<-82) and extreme ==0:
                    BOOOL=True                 
                    try:
...
##################################################################################################################################################################################################################################### ICONS CODE(FYP)
            ...
            if((now - begin)>15 and controller._autopilot_enabled == False ):
                ...
                try:
                    ...
                    i="love"
                      #########################fyp switch to autopilot message
         #################fyp bug solved
