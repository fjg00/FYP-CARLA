...
'''
                ...
                if event.button == 0:
                    ...
                    if parking==False:
                        ...
                        parking=False
                    #world.restart()
                    #if (world.player.get_transform().location.x>-90 and world.player.get_transform().location.x<-85 and world.player.get_transform().location.y>-42 and world.player.get_transform().location.y<-39):
                        # while True:
                        #     world.player.apply_control(carla.VehicleControl(throttle=0.5, steer=0.6, reverse=True))
                        #     #time.sleep(0.1)
                        #     if world.player.get_transform().location.y <-45:
                        #         break
                        # print("TEKRAM333333")
...
def obstacledetected(weak_self,event):
    selfs = weak_self()
    if not selfs:
        return
    selfs.distance = event.distance
    selfs._event_count += 1
    icon = pygame.image.load('/home/justin/Downloads/cat.jpeg')
    display.blit(icon, (900, 900))    
    pygame.display.set_icon(icon) 
    print ("Event %s, in line of sight with %s at distance %u" % (selfs._event_count, event.other_actor.type_id, event.distance))

def obstacle(ssensor,weak_self):               
    ssensor.listen(lambda event: ssensor.obstacledetected(weak_self, event))                        
...
def game_loop(args):
    ...
    ##### points Transform(Location(x=-85.998741, y=22.898359, z=0.048869), Rotation(pitch=0.579719, yaw=82.274673, roll=0.116839))   Transform(Location(x=-84.941872, y=47.483334, z=0.508699), Rotation(pitch=1.700990, yaw=88.209892, roll=-0.049072))  Transform(Location(x=-84.802010, y=28.449863, z=0.093546), Rotation(pitch=0.648376, yaw=92.703255, roll=0.031441))
        ...
        while True:
            ...
            # file2.write("z = "+str(coord.location.z)+"\n")
            # file2.write("pitch "+str(coord.rotation.pitch)+"\n")
            # file2.write("yaw "+str(coord.rotation.yaw)+"\n")
            # file2.write("roll "+str(coord.rotation.roll)+"\n")
            
            # file2.write("\n")
            
            ####Marina
            
            # marina_read = time.time()
            
            # if ((marina_read - hassoun_start)>30):
            #     coord = (world.player.get_transform())
            #     coord_details= [coord.location.x, coord.location.y, coord.location.z, coord.rotation.pitch, coord.rotation.yaw, coord.rotation.roll]
            #     with open('/home/justin/Documents/trialsem2.csv', 'a') as f:
            #         writer = csv.writer(f)
            #         writer.writerow(coord_details)
            ...
            # if startred == 1:
            #     b = time.time()
            #     x = numpy.random.randint(3,high = 10)
            #     y = numpy.random.randint(3,high = 10)
            #     startred = 0
            #     startgreen = 0
            ...
            #          Traffic_light_3.set_state(carla.TrafficLightState.Red)
            #          icon = pygame.image.load('/home/justin/Downloads/cat.jpeg')
            #          display.blit(icon, (900, 900))    
            #          pygame.display.set_icon(icon) 
            #     else: 
            ...
            ###################################################################################first intersection
            #Traffic_light_1.set_state(carla.TrafficLightState.Green)         #######################################################################li bl wejjjjj
...
##################################################################################################################################################################################################################################### ICONS CODE(FYP)
            ...
            icon = pygame.image.load('/home/justin/Desktop/parking_on.png')
            display.blit(icon, (700, 530))    
            pygame.display.set_icon(icon) 
            ...
            # icon = cv2.imread('/home/justin/Documents/ipad.png')
            # icon = cv2.resize(icon,(510,300))
            # cv2.imwrite('/home/justin/Documents/ipad2.png',icon)
            ...
            # if(time.time()- last_update>delay_update):
            #     try:
            #         last_update = time.time()
            #         icon = cv2.imread('/home/justin/Downloads/FYP_icons/renders\/render.jpg')
            #         icon = cv2.resize(icon,(325,250))
            #         cv2.imwrite('/home/justin/Downloads/FYP_icons/render1.jpg',icon)
            #     except:
            #         print("NO IMAGES")
            # try:
            #     if Marc == 1:
            #         icon = pygame.image.load('/home/justin/Downloads/FYP_icons/render1.jpg')              
            #         rect = icon.get_rect()
            #         rect.center = 1550, 870
            #         icon.convert()  
            #         display.blit(icon, rect)
            ...
            try:
                ...
                if np.count_nonzero((results_array==1) | (results_array==2))>=4:
                ...
                if(radio==0):
                    icon = pygame.image.load('/home/justin/Desktop/radio_small.png')
                    display.blit(icon, (1070, 800))    
                    pygame.display.set_icon(icon) 
            #else:
            #    if ((time.time()-lowspeedstart)>10):
            #        icon = pygame.image.load('/home/justin/Downloads/trial.png')

            #        display.blit(icon, (1550, 780))    
            #        pygame.display.set_icon(icon) 
            ...
            #     cam_bp.set_attribute("image_size_y",str(1080))
            #     cam_bp.set_attribute("fov",str(105))
            #     cam_bp.set_attribute("sensor_tick",str(1.2))
            #     cam_transform = carla.Transform(carla.Location(x=-3, z=10),carla.Rotation(-90,0,0))
            #     ego_cam = world.world.spawn_actor(cam_bp,cam_transform,attach_to=world.player, attachment_type=carla.AttachmentType.Rigid)
            #     first_Time = 0
            ...
            if (time.time()-last > delay):
                ...
                icon = pygame.image.load('/home/justin/Downloads/FYP_icons/loading.png')
                display.blit(icon, (1550, 780))    
                pygame.display.set_icon(icon)
            ...
            speed=3.6 * math.sqrt(V.x**2 + V.y**2 + V.z**2)
            saved_points = [steer,throttle,brake,reverse,speed]
            # if parking==1:
            #     with open('/home/justin/Documents/parking/2.csv', 'a') as f:
            #         writer = csv.writer(f)
            ...
            if (time.time()-last2 > delay2):
                ...
                if (firstt_Time == 0):
                    ...
                    obs_sensor = world.world.spawn_actor(bp, carla.Transform(), attach_to=world.player, attachment_type=carla.AttachmentType.Rigid)
                    weak_self = weakref.ref(obs_sensor)
                    firstt_Time = 0
                    t4 = Thread(target=obstacle,args=[obs_sensor,weak_self])
                    t4.start()
