...
class World(object):
    ...
    def restart(self):
        ...
        if self.player is not None:
            ...
            print(self.player.get_attribute)
...
# -- RadarSensor ---------------------------------------------------------------
# ==============================================================================
def threadd(speedo,brako,throttlo,loaded_model,s, display):
    print("FATIMAAAAAAAAAAAAAAAAAAA", len(speedo) )
    global result
    if (len(speedo)>=200):

        print("MARINAAAAAAA")
        X= np.column_stack((brako[-200:],throttlo[-200:],speedo[-200:]))
        #pca = PCA(n_components=2)
        #X=pca.fit_transform(X)
        X = np.array([X])
        print("MARKKKKKKKKKKKKKKKKKKK", X)
        result = loaded_model.predict(X)
        print("predddddddddddD: ",result)
...
def game_loop(args):
    ...
    try:
        ...
        image_green = Image.open("/home/justin/Downloads/M_Interior_d_green.TGA")
        image_blue = Image.open("/home/justin/Downloads/M_Interior_d_blue.TGA")
        height_green = image_green.size[1]
        width_green = image_green.size[0]
        height_blue = image_blue.size[1]
        width_blue = image_blue.size[0]

        texture_green = carla.TextureColor(width_green ,height_green)
        texture_blue = carla.TextureColor(width_blue ,height_blue)
        for x in range(0,width_green):
            for y in range(0,height_green):
                color = image_green.getpixel((x,y))
                r = int(color[0])
                g = int(color[1])
                b = int(color[2])
                a = 255
                texture_green.set(x, y, carla.Color(r,g,b,a))

        for x in range(0,width_blue):
            for y in range(0,height_blue):
                color = image_blue.getpixel((x,y))
                r = int(color[0])
                g = int(color[1])
                b = int(color[2])
                a = 255
                texture_blue.set(x, y, carla.Color(r,g,b,a))
    ...
    ##### points Transform(Location(x=-85.998741, y=22.898359, z=0.048869), Rotation(pitch=0.579719, yaw=82.274673, roll=0.116839))   Transform(Location(x=-84.941872, y=47.483334, z=0.508699), Rotation(pitch=1.700990, yaw=88.209892, roll=-0.049072))  Transform(Location(x=-84.802010, y=28.449863, z=0.093546), Rotation(pitch=0.648376, yaw=92.703255, roll=0.031441))
        ...
        # with open('/home/justin/Documents/trial.csv', 'a') as f:
        #     writer = csv.writer(f)
        ...
        timo =[]
        prevv=0
        filepath= "/home/justin/Downloads/model2.hdf5"
        ...
        while True:
            ...
            # if ((marina_read - hassoun_start)>10):
            #     coord = (world.player.get_transform())
...
##################################################################################################################################################################################################################################### ICONS CODE(FYP)
            ...
            brako.append(C.brake)
            speedo.append(speed)
            throttlo.append(C.throttle)

            
            try:
                    if result >= 0.6:
                            icon = pygame.image.load('/home/justin/Downloads/trafficc.jpeg')
                            display.blit(icon, (800, 800))    
                            pygame.display.set_icon(icon) 
                            pygame.display.flip()  
                    elif result<=0.3:
                            icon = pygame.image.load('/home/justin/Downloads/no_traffic.jpg')
                            display.blit(icon, (800, 800))    
                            pygame.display.set_icon(icon) 
                            pygame.display.flip()  
            except:
            ...
            if (time.time()-last > delay):
                delay = 5
                last = time.time()
                t1 = Thread(target=threadd,args=(speedo,brako,throttlo,loaded_model,s,display))
                t1.start()
