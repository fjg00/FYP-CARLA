    ...
    def restart(self):
        ...
        while self.player is None:
           ...
           # spawn_point = random.choice(spawn_points) if spawn_points else carla.Transform()
            spawn_point= carla.Transform(carla.Location(x=-129.991348, y=0.500894, z=0.129238), carla.Rotation(pitch=0, yaw=0, roll=0))
            self.player = self.world.try_spawn_actor(blueprint, spawn_point)
            print(self.player)
            self.modify_vehicle_physics(self.player)
        # Set up the sensors.
...
class CameraManager(object):
 ...
 #   def setup_depth_camera(self):
        ...
        Sets calibration for client-side boxes rendering.depth_camera_transform =(carla.Transform(carla.Location(x=0.28, y=-0.45, z=1.2), carla.Rotation(0,55)))
...
x=-0.15,y=-0.4, z=1.2
        ...
        """
  #      Attachment = carla.AttachmentType
   #     depth_camera_transform =(carla.Transform(carla.Location(x=-0.15,y=-0.4, z=1.2), carla.Rotation(yaw=-70)))
    #    self.depth_camera = self._parent.get_world().spawn_actor(self.depth_camera_blueprint(), depth_camera_transform, attach_to= self._parent)
     #   weak_depth_self = weakref.ref(self)
     #   self.depth_camera.listen(lambda depth_image: CameraManager._set_depth_image(weak_depth_self, depth_image))
    ...
    #def setup_depth2_camera(self):
        ...
        """
     #   Attachment = carla.AttachmentType
      #  depth2_camera_transform =(carla.Transform(carla.Location(x=-0.15,y=-0.4, z=1.2), carla.Rotation(yaw=70)))
       # self.depth2_camera = self._parent.get_world().spawn_actor(self.depth_camera_blueprint(), depth2_camera_transform, attach_to= self._parent)
       # weak_depth2_self = weakref.ref(self)
        #self.depth2_camera.listen(lambda depth2_image: CameraManager._set_depth2_image(weak_depth2_self, depth2_image))

        #calibration = np.identity(3)
        #calibration[0, 2] = VIEW_WIDTH / 2.0
        #calibration[1, 2] = VIEW_HEIGHT / 2.0
        #calibration[0, 0] = calibration[1, 1] = VIEW_WIDTH / (2.0 * np.tan(VIEW_FOV * np.pi / 360.0))
        #self.depth2_camera.calibration = calibration
#JASON
...
#JASON
    ...
    #def depth2_render(self, depth2_display):
     #   if self.depth2_image is not None:
      #      i = np.array(self.depth2_image.raw_data)
       #     i2 = i.reshape((1080,1920, 4))
        #    i3 = i2[:, :, :3]
         #   self.depth2 = i3
          #  cv2.imshow("depth2_image", self.depth2)
#JASON#
    @staticmethod
    def _parse_image(weak_self, image):
     ...
     #JASON
    #def _set_depth_image(weak_depth_self, depth_img):
        """
    ...
    #def _set_depth2_image(weak_depth2_self, depth2_img):
        """
        Sets image coming from camera sensor.
        The self.capture flag is a mean of synchronization - once the flag is
        set, next coming image will be stored.
     ...
     #   self = weak_depth2_self()
      #  if self.depth2_capture: 
       #     self.depth2_image = depth2_img
        #    self.depth2_capture = False
#JASON
# ==============================================================================
# -- game_loop() ---------------------------------------------------------------
# ==============================================================================
...
def game_loop(args):
    ...
    try:
        ...
        spawn_point3= carla.Transform(carla.Location(x=-53.396687, y=131.291458), carla.Rotation(pitch=0.000082, yaw=178.904358, roll=0.000004))
        ...
        #p3=world.world.try_spawn_actor(blueprint, spawn_point3)
        ...
        Traffic_light_2= world.world.get_actors().filter('traffic.traffic_light')[11]
        ...
        while True:
            ...
            print(world.player.get_transform().location.x)
            ...
            if Traffic_Light_id== Traffic_light_1_id or Traffic_Light_id==Traffic_light_1_id:
            ...
            print(Traffic_Light)
            #a=world.player.get_traffic_light_state()
            #print(a)
            #print(Traffic_light_id_list)
            #print(len(Traffic_light_id_list))
            #if Traffic_Light!=None:
             #   Traffic_Light_id=Traffic_Light.id

            
            #if Traffic_Light_id not in Traffic_light_id_list:
            #    Traffic_light_id_list.append(Traffic_Light_id)
            #if len( Traffic_light_id_list)==3:
            ...
            # print(world.player.get_traffic_light_state())
                ...
                if (world.player.get_transform().location.x>-110 and world.player.get_transform().location.x<-95 and world.player.get_transform().location.y>-2 and world.player.get_transform().location.y<2):
                    if List[36].state== carla.TrafficLightState.Red:
                        hud.notification("Red Light Detected")
                    elif List[36].state== carla.TrafficLightState.Green:
                        hud.notification("Green Light Detected")
                '''if (world.player.get_transform().location.x>235 and world.player.get_transform().location.x<250 and world.player.get_transform().location.y>15 and world.player.get_transform().location.y<90):
                    if List[9].get_traffic_light_state()== carla.TrafficLightState.Red:
                        hud.notification("Red Light Detected")
                    elif List[9].get_traffic_light_state()== carla.TrafficLightState.Green:
                        hud.notification("Green Light Detected")'''
               ...
               #  p4=world.world.try_spawn_actor(blueprint, spawn_point4)
                ...
                # p6=world.world.try_spawn_actor(blueprint, spawn_point6)
                    ...
                    #try:
                     #  p5.set_autopilot(True)
                      # tm.ignore_lights_percentage(p5,100)
                    #except:
                     #   print("")

                       #traffic_light5=p5.get_traffic_light()
                    ...
                    try:
                       ...
                       tm.ignore_lights_percentage(p6,100)
                        ...
                        print("")
                       #tm.ignore_lights_percentage(p6,100)
                #if (world.player.get_transform().location.x>-90 and world.player.get_transform().location.x<-84 and world.player.get_transform().location.y>110 and world.player.get_transform().location.y<120):
...
#
            ...
            elif Changed and (time.time()-start)<2:
                print("FF")
                world.player.set_autopilot(False)
                a=carla.VehicleControl()
                a.brake=1
