...
def game_loop(args):
    ...
    command = "espeak -v en+f3 'Traffic around you detected, feel free to press on triangle to let me drive for you'"
    try:
        global Changed
        Changed=False
        Start=time.time()
...
##################################################################################################################################################################################################################################### ICONS CODE(FYP)
            ...
            # try:
            if speak == 1  and say_it == 1 and Auto == 0:
                geometry = "0x0+0+0"
                # threading.Thread(target=os.system, args=(f"espeak -v en+f3 'Traffic around you detected, feel free to press on triangle to let me drive for you'",)).start()
                subprocess.Popen(['gnome-terminal','--hide-menubar','--geometry',geometry,'-e',"espeak -v en+f3 'there is traffic around you... feel free to press on triangle... to let me drive for you'"])
